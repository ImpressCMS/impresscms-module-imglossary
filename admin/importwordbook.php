...
function DefinitionImport( $delete ) {
    global $icmsConfig, $entryID, $myts;
    $sqlquery = icms::$xoopsDB -> query( "SELECT COUNT(entryID) AS count FROM " . icms::$xoopsDB -> prefix( 'wbentries' ) );
    ...
    $sql1 = icms::$xoopsDB -> query( "SELECT * FROM " . icms::$xoopsDB -> prefix( 'wbentries' ) . " " );
    ...
    $sql2 = icms::$xoopsDB -> query( "SELECT * FROM " . icms::$xoopsDB -> prefix( 'wbcategories' ) . " ORDER BY categoryID" );

    $result2 = icms::$xoopsDB -> getRowsNum( $sql2 );
    if ( $result2 ) {
        while ( $row1 = icms::$xoopsDB -> fetchArray( $sql2 ) ) {
            ...
            if ( $delete ) {
                $ret2 = icms::$xoopsDB -> queryF( "INSERT INTO " . icms::$xoopsDB -> prefix( 'imglossary_cats' ) . " (categoryID, name, description, total, weight) VALUES ('$categoryID','$name', '$description', '$total', '$weight')" );
            } else {
                $ret2 = icms::$xoopsDB -> queryF( "INSERT INTO " . icms::$xoopsDB -> prefix( 'imglossary_cats' ) . " (categoryID, name, description, total, weight) VALUES ('', '$name', '$description', '$total', '$weight')" );
            }
            if ( !$ret2 ) {
                $errorcounter = $errorcounter + 1;
    ...
    $sqlquery3 = icms::$xoopsDB -> query( "SELECT mid FROM " . icms::$xoopsDB -> prefix( 'modules' ) . " WHERE dirname='wordbook'" );
    list( $wbkID ) = icms::$xoopsDB -> fetchRow( $sqlquery3 );

    echo "<p>Wordbook Module ID: " . $wbkID . "</p>";
    echo "<p>imGlossary Module ID: " . icms::$module -> getVar( 'mid' ) . "</p>";
    //echo "<p>delete is on/off: ".$delete."</p>";

    $commentaire = icms::$xoopsDB -> queryF( "UPDATE " . icms::$xoopsDB -> prefix( 'xoopscomments' ) . " SET com_modid = '" . icms::$module -> getVar( 'mid' ) . "' WHERE com_modid='" . $wbkID . "'" );
...
function FormImport() {
    ...
    if ( is_object( $wordbookModule ) ) {
        ...
        echo "<tr>";
 //       echo "<td class='head' width = '200' align='center'><img src='" . ICMS_URL . "/modules/" . icms::$module -> getVar('dirname') . "/images/icon/exclamation.png"."' alt='' hspace='0' vspace='0' align='middle' style='margin-right: 10px; '></td>";
